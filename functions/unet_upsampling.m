...
function [newlgraph,outputName] = unet_upsampling(lgraph, inputName)

[lgraph,outputName] = ResDownBlock(lgraph, inputName,'FilterSize',9,'numFilters',16,'DilationFactor',3, 'Order',1); 
[lgraph,outputName] = ResDownBlock(lgraph,outputName,'FilterSize',7,'numFilters',32,'DilationFactor',2, 'Order',2);
...
[lgraph,outputName] = ResDownBlock(lgraph,outputName,'FilterSize',3,'numFilters',128,'DilationFactor',1, 'Order',5);
[lgraph,outputName] = ResDownBlock(lgraph,outputName,'FilterSize',3,'numFilters',160,'DilationFactor',1, 'Order',6);
[lgraph,outputName] = ResUpBlock(lgraph,outputName,'FilterSize',9,'numFilters',128,'Order',5);
[lgraph,outputName] = ResUpBlock(lgraph,outputName,'FilterSize',7,'numFilters',96,'Order',4);
...
lgraph = connectLayers(lgraph,'ResDAdd5','ResUAdd5/in3');
...
Layers = [
    ...
    convolution2dLayer(filterSize,numFilters, 'DilationFactor',dilationFactor,'Stride',2,'Padding','same','PaddingValue','symmetric-include-edge','Name',['ResDownConv' k '_1'])
    batchNormalizationLayer('Name',['ResDownBN' k '_2'])
    reluLayer('Name',['ResDownReLU' k '_2'])
    convolution2dLayer([3 3],numFilters, 'Padding','same','PaddingValue','symmetric-include-edge','Name',['ResDownConv' k '_2'])
    additionLayer(2,'Name',['ResDownAdd' k])
    
    batchNormalizationLayer('Name',['ResDBN' k '_1'])
    reluLayer('Name',['ResDReLU' k '_1'])
    convolution2dLayer([3 3],numFilters,'Stride',[1 1],'Padding','same','PaddingValue','symmetric-include-edge','Name',['ResDConv' k '_1'])
    batchNormalizationLayer('Name',['ResDBN' k '_2'])
    reluLayer('Name',['ResDReLU' k '_2'])
    convolution2dLayer([3 3],numFilters,'Stride',[1 1],'Padding','same','PaddingValue','symmetric-include-edge','Name',['ResDConv' k '_2'])
    additionLayer(2,'Name',['ResDAdd' k])];

lgraph = addLayers(lgraph,Layers);
lgraph = connectLayers(lgraph,inputName,['ResDownBN' k '_1']);

skip = convolution2dLayer([1 1],numFilters,'Stride',[2 2],'Padding','same','PaddingValue','symmetric-include-edge','Name',['SkipDownConv' k]);
...
Layers = [
    ...
    resize2dLayer('Name',['ResUpResize' k ],'Scale',[2,2],'Method','nearest')
    convolution2dLayer([3 3],numFilters,'Stride',[1 1],'Padding','same','PaddingValue','symmetric-include-edge','Name',['ResUpConv' k '_1'])
    batchNormalizationLayer('Name',['ResUpBN' k '_2'])
    reluLayer('Name',['ResUpReLU' k '_2'])
    convolution2dLayer([3 3],numFilters,'Stride',[1 1],'Padding','same','PaddingValue','symmetric-include-edge','Name',['ResUpConv' k '_2'])
    additionLayer(2,'Name',['ResUpAdd' k])
    
    batchNormalizationLayer('Name',['ResUBN' k '_1'])
    reluLayer('Name',['ResUReLU' k '_1'])
    convolution2dLayer([3 3],numFilters,'Stride',[1 1],'Padding','same','PaddingValue','symmetric-include-edge','Name',['ResUConv' k '_1'])
    batchNormalizationLayer('Name',['ResUBN' k '_2'])
    reluLayer('Name',['ResUReLU' k '_2'])
    convolution2dLayer([3 3],numFilters,'Stride',[1 1],'Padding','same','PaddingValue','symmetric-include-edge','Name',['ResUConv' k '_2'])
    additionLayer(numInputs,'Name',['ResUAdd' k])];

lgraph = addLayers(lgraph,Layers);
lgraph = connectLayers(lgraph,inputName,['ResUpBN' k '_1']);

skip = [ resize2dLayer('Name',['SkipUpResize' k ],'Scale',[2,2],'Method','nearest')
         convolution2dLayer([1 1],numFilters,'Stride',[1 1],'Padding','same','PaddingValue','symmetric-include-edge','Name',['SkipUpConv' k ])];
lgraph = addLayers(lgraph,skip);

lgraph = connectLayers(lgraph,inputName,['SkipUpResize' k]);
